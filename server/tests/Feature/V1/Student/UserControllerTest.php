...
namespace Tests\Feature\V1\Student;
...
class UserControllerTest extends TestCase
{
    ...
    public function test_register()
    {
        ...
        foreach ($cases as $case) {
            $response = $this->post(route('student.v1.auth.register'), $case['input']);
    ...
    public function test_login()
    {
        ...
        foreach ($cases as $case) {
            $response = $this->post(route('student.v1.auth.login'), $case['input']);
    ...
    public function test_password_verify()
    {
        ...
        $token = \App\Models\Student::find(1)->createToken('phpunit')->accessToken;
        $sesionIdentifier = \App\Models\StudentSession::find(1)->identifier;
        ...
        $cases = [
            ...
            [
                ...
                'header' => [
                    'Authorization' => $header['Authorization'],
                    'sessionIdentifier' => \App\Models\StudentSession::find(2)->identifier,
                ],
                'response' => [
                    'status' => false,
                    'code' => 401,
        ...
        foreach ($cases as $case) {
            ...
            $response = $this->post(route('student.v1.auth.verifyPassword'), $case['input'], $requestHeader);
            $response->assertStatus($case['code']);
            $response->assertJson($case['response']);
        }
    }
    ...
    public function test_get_profile()
    {
        ...
        $cases = [
            [
                'header' => [
                    'Authorization' => $header['Authorization'],
                    'sessionIdentifier' => \App\Models\StudentSession::find(3)->identifier,
                ],
                'response' => [
                    'status' => false,
                    'code' => 403,
        ...
        foreach ($cases as $case) {
            $requestHeader = array_merge($case['header'], $this->header);
            $response = $this->get(route('student.v1.auth.profile'), $requestHeader);
    ...
    public function test_verify_account()
    {
        ...
        foreach ($cases as $case) {
            $response = $this->post(route('student.v1.auth.verifyAccount'), $case['input'], $this->header);
